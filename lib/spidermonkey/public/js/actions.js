function ajaxErrorHandler(jqXHR, textStatus, errorThrown) { // TODO - not working
  $("#error_bar").remove();
...
function sendMsg(e) {
  if (typeof animateManager === "function") {
    ...
    parent.postMessage(JSON.stringify(e), location.protocol + "//" + location.host);
  }
}
...
function actionSave(href) {
  $.ajax({
    ...
    url: href,
    success: function(data) {
      alert("Item saved successfully to SimpleDB");
    },
    error: ajaxErrorHandler
  });
}
