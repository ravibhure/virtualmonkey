(function($) {
  ...
  */
          ...
          var drawSpreadsheet = function(record) {
            ...
            switch(record["status"]) {
              ...
              case "pending":
                ...
                break;
              case "blocked":
                cell.removeClass().addClass("blocked");
                cell.html(report_page);
                break;
              default:
                break;
  ...
  function animateManager(data) {
    slider_view = $("#manager_slider");
    scrollDistance = slider_view.innerHeight() + parseInt(slider_view.css("padding-bottom"));
    switch (data.action) {
      case "createedit":
  ...
  $("#manager_modal").draggable({handle: "div.modal-header, div.modal-footer"});
